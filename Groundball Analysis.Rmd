...
updated_pitchers_groundballs <- updated_pitchers[, c(4, 7:20, 22:57, 59, 61, 68, 70:139)]
...
bag_mod <- randomForest(groundballs ~., # Set tree formula
                ...
                mtry = 123, # Set mtry to number of variables 
...
bag_mod_2 <- randomForest(groundballs ~., # Set tree formula
                data = train_data, # Set dataset
                mtry = 123, # Set number of variables to try  
...
dtrain <- xgb.DMatrix(data = as.matrix(train_data[, c(1:52, 54:124)]), label = train_data$groundballs)
# Create test matrix
dtest <- xgb.DMatrix(data = as.matrix(test_data[, c(1:52, 54:124)]), label = test_data$groundballs)
```
...
shap_result <- shap.score.rank(xgb_model = bst_1, 
                X_train =as.matrix(train_data[, c(1:52, 54:124)]),
                shap_approx = F)
...
elite_pitchers_2017 <- pitchers_2017[rownames(scale_pitchers_2017) %in%
                                     ...
                                     colnames(scale_pitchers_2017) %in%
                                       c("last_name","first_name","edge","out_zone","in_zone",
                                         "poorlyunder_percent","pitch_count","in_zone_swing",
                                         "out_zone_swing","poorlytoppd_percent","pitch_count_fastball",
                                         "n_ff_formatted","z_swing_miss_percent","whiff_percent")]
